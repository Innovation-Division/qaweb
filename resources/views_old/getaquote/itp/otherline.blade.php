...
btnNextPage.click(function(){
    ...
    if(promo != ""){
        url = url  + '/api/get-quote/itp-insurance/promo';
        $.ajax({
            url: url,
            method:"GET",
        ...
        if(result.length == 0){
                ...
                return false;
                    ...
                    var package = $('#package').val();
                    var return_date_itinerary = $('#return_date_itinerary').val();
                    var include_cruise = $('input[name="include_cruise"]:checked').val();
                    ...
                    $('#quoteprovincecruise_final').text(destinations_cruise);
                    $('#quotecruise_final_provincecruise').text(destinations_cruise);


                    var urlwithpromo = jQuery('input[name="url"]').val();
                    var urlprem = urlwithpromo  + '/api/get-quote/itp-insurance/get-prem';
                    $.ajax({
                        url: urlprem,
                        method:"GET",
                        data:{ _token:_token,destinations:destinations,noofday:noofday,cruise:cruise,covid:covid,package:package},
                        success:function(result)
                        {

                            $.each(result, function(key, value){
                                // jQuery('#continent').val(result[0]['continent']);
                                        ...
                                        dst = 0;
                                        if(package == "Economy"){
                                            prem_one = '10000.00 US$' ;
                                            prem_two = '250.00 US$';

                                            dst = 2.00;
                                        }else if (package == "Esteem"){
                                            prem_one = '25000.00 US$' ;
                                            prem_two = '500.00 US$';
                                            dst = 4.00;
                                        }else if (package == "Executive"){
                                            prem_one = '50000.00 US$' ;
                                            prem_two = '1000.00 US$'  ;
                                            dst = 4.00;
                                        }else{
                                            prem_one = '100000.00 US$'  ;
                                            prem_two = '2000.00 US$'  ;
                                            dst = 4.00;
                                        }
                                        ...
                                        duetotal = parseFloat(premium) + premtax + dst + lgt;
                                ...
                                if(covid == "Yes"){
                                      urlpremcovid = urlwithpromo  + '/api/get-quote/itp-insurance/get-prem-covid';
                                            $.ajax({
                                            url: urlpremcovid,
                                            method:"GET",
                                           ...
                                           jQuery.each(data, function(key, value){
                                            ...
                                            success:function(result)
                                            {
                                                $.each(result, function(key, value){
                                                    ...
                                                    dst = 4.00;
                                                    premium = parseFloat(premium) + parseFloat(prem);
                                                    jQuery('input[name="covid_amount"]').val(prem);
                                                    ...
                                                    if(promo != ""){
                                                      ...
                                                      urlpromo = urlwithpromo  + '/api/get-quote/itp-insurance/promo';
                                                      $.ajax({
                                                            url: urlpromo,
                                                            method:"GET",
                                                            data:{ _token:_token,promo:promo},
        ...
        if(Date.parse(from_cruise) > Date.parse(to_cruise)){
          ...
          //$("#departure_date_itinerary").after("<div class='validation_departure_date_itinerary v-error-msg'>Invalid Date Range</div>");
             //$('#departure_date_itinerary').fieldErrorState();
            $("#return_date_cruise_itinerary").after("<div class='validation_return_date_itinerary v-error-msg'>Return cruise date should later than the departure cruise date2 </div>");
        ...
        if(Date.parse(from) > Date.parse(from_cruise) || Date.parse(to) < Date.parse(from_cruise)){
          ...
          //$("#departure_date_itinerary").after("<div class='validation_departure_date_itinerary v-error-msg'>Invalid Date Range</div>");
             //$('#departure_date_itinerary').fieldErrorState();
            $("#departure_date_cruise_itinerary").after("<div class='validation_return_date_itinerary v-error-msg'>departure cruise date should be in between of departure date </div>");
        ...
        if(Date.parse(from) > Date.parse(to_cruise) || Date.parse(to) < Date.parse(to_cruise)){
          ...
          //$("#departure_date_itinerary").after("<div class='validation_departure_date_itinerary v-error-msg'>Invalid Date Range</div>");
             //$('#departure_date_itinerary').fieldErrorState();
            $("#return_date_cruise_itinerary").after("<div class='validation_return_date_itinerary v-error-msg'>Return cruise date should be in between of departure date </div>");
            $('#return_date_cruise_itinerary').fieldErrorState();
        }else{
        ...
        if(Date.parse(from) > Date.parse(to)){
          ...
          //$("#departure_date_itinerary").after("<div class='validation_departure_date_itinerary v-error-msg'>Invalid Date Range</div>");
             //$('#departure_date_itinerary').fieldErrorState();
            $("#return_date_cruise_itinerary").after("<div class='validation_return_date_cruise_itinerary v-error-msg'>Return date should later than the departure date2 </div>");
            $('#return_date_cruise_itinerary').fieldErrorState();
        }
                   ...
                   jQuery.each(data, function(key, value){
                    ...
                    success:function(result)
                    {
                        ...
                        $.each(data, function(key, value){
                            ...
                            console.log(value);
                            console.log('DITO NA SIMULA');
                       

                            var continent = value.continent;
                               var noofdaysorig = value.days;
                                     console.log('DITO NA ULIT');
                                     console.log('No. of Days ' + noofdaysorig);
                                if(data.length == 0){
                                // alert("0");
                                    }else{
                                    ...
                                    // console.log('due total ' + duetotal + ' dst ' + premtax + 'lgt ' + lgt + 'premtax ' + premtax);
                                      ...
                                      var errors = data.responseJSON;
                                       ...
                                       jQuery.each(data, function(key, value){
                                        ...
                                        success:function(result)
                                        {
                                            $.each(result, function(key, value){
                                                console.log(value);
                                                console.log(prem+'--'+value.prem+' FROM COvID PREM ');
                                                // jQuery('#withcovid').show();
                                                // jQuery('#withcovid_final').show();
                                                ...
                                                console.log('PREMVALUE '+ parseFloat(prem) +'NETPREM '+ premtax_com + 'DST' + dst + 'LGT' + lgt_com);
