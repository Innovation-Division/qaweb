                        ...
                        <div class="col-12 col-xl-10">
                            <div id="articleList" class="articleList">
                                    ...
                                    <form class="form-default">
                                        <label for="article-search">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="20.341" height="20.341" viewBox="0 0 20.341 20.341">
                                                <path id="Path_1687" data-name="Path 1687" d="M17.537,15.793h-.919l-.326-.314a7.571,7.571,0,1,0-.814.814l.314.326v.919l5.815,5.8,1.733-1.733Zm-6.978,0a5.233,5.233,0,1,1,5.233-5.233A5.226,5.226,0,0,1,10.559,15.793Z" transform="translate(-3 -3)" fill="teal" />
                                            </svg>
                                        </label>
                                ...
                                <div class="listContainer">
                                    <div class="list row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4 mb-4"></div>
                                </div>
                ...
                <div class="container">
                    <div class="row align-items-center justify-content-center">
                        <div class="col-12 col-md-5 text-center">
                            <a href="{{ url('/blogs') }}" class="newsSelectionItem d-block blog-bg">
...
@section('before_body_end_scripts')
    ...
    <script>
        var values = [
			@foreach($cocogen_admin_pages_news as $cocogen_admin_pages_new)
			@php
				$summary = str_replace( array( "\n", "\r" ), array( "\\n", "\\r" ), Str::limit($cocogen_admin_pages_new->summary, 100, ' ...') );
			@endphp
            {
                image: "@if($cocogen_admin_pages_new->thumbImage) {{ $cocogen_admin_pages_new->thumbImage }} @else {{ asset('/assets/img/526x370.jpg') }} @endif",
                date: "{!! date('F j, Y', strtotime($cocogen_admin_pages_new->created_at)) !!}",
                title: "{{ \Illuminate\Support\Str::limit($cocogen_admin_pages_new->name, 50, $end='...') }}",
                link: "{{ url('/') }}/{{ $cocogen_admin_pages_new->link }}",
                linkTitle: "{{ url('/') }}/{{ $cocogen_admin_pages_new->link }}",
                description: "{{$summary}}",
                readButton: "{{ url('/') }}/{{ $cocogen_admin_pages_new->link }}"
            }
            @if(!$loop->last),@endif
			@endforeach
        ];
        ...
        var options = {
            valueNames: [
                ...
                "date",
                { name: 'link', attr: 'href' },
                { name: 'linkTitle', attr: 'href' },
                "title",
                "description",
                { name: 'readButton', attr: 'href' }
            ],
            item:
            '<div class="col">\
                <div class="card h-100">\
                    ...
                    <div class="card-footer">\
                        <a href="#" class="readButton btn btn-secondary-light">Read Article</a>\
                    </div>\
                </div>\
            </div>',
            page: 6,
