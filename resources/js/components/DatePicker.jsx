...
const DatepickerWithOptionalMinMax = ({
    ...
    // Normalize minDate and maxDate to start of day for accurate comparison
    ...
    const getDisplayedDaysInfo = (month, year) => {
        ...
        const firstDayOfMonth = getFirstDayOfMonth(month, year); // 0 (Sun) - 6 (Sat)
        ...
        // Calculate how many trailing days are needed to complete the last row
        // If totalDaysDisplayedSoFar is exactly a multiple of 7, we need a new row (7 days)
        // Otherwise, we need (7 - (totalDaysDisplayedSoFar % 7)) days to fill the current row
    ...
    const handleInputChange = (event) => {
        ...
        // You might want to add date parsing/validation here if the user can type dates
    };

    const handleInputFocus = () => {
        setIsOpen(true);
        // When opening, ensure tempSelectedDate matches selectedDate
        if (selectedDate && tempSelectedDate) {
        ...
        } else if (!selectedDate && !tempSelectedDate) {
            // If no date is selected, initialize to current month/year view
            setCurrentMonth(new Date().getMonth());
            setCurrentYear(new Date().getFullYear());
        }
    };
    ...
    const handleApplyClick = (e) => {
        e.preventDefault();
        if (tempSelectedDate) {
            // Check min/max one last time before applying
            const isWithinMin =
                !normalizedMinDate || tempSelectedDate >= normalizedMinDate;
            const isWithinMax =
                !normalizedMaxDate || tempSelectedDate <= normalizedMaxDate;
            ...
            if (isWithinMin && isWithinMax) {
                ...
                // Handle case where tempSelectedDate somehow became invalid (shouldn't happen with proper disabling)
                setSelectedDate(null);
                setInputValue("");
                onDateSelect(null);
            }
    ...
    const handleCancelClick = () => {
        setIsOpen(false);
        // Revert temp selection to the last confirmed selectedDate
        setTempSelectedDate(selectedDate);
    };

    // Effect for handling outside clicks to close the datepicker
    useEffect(() => {
    ...
    const getDynamicYearOptions = () => {
        ...
        let startYear = currentYearDefault;
        let endYear = currentYearDefault + 10;

        if (normalizedMinDate) {
            startYear = normalizedMinDate.getFullYear();
        }

        if (normalizedMaxDate) {
            endYear = normalizedMaxDate.getFullYear();
        }
