...
const Toast = (title, message) => {
    const PlanSelectedToast = ({ closeToast }) => (
        <div className="flex justify-around gap-3 w-full py-2">
            <style
                dangerouslySetInnerHTML={{
                    __html: `
               .Toastify__progress-bar-theme--light {
    background: #09A12A !important;
}
          `,
                }}
            />
            ...
            <div className="flex flex-col w-full gap-1">
                <p className="font-bold text-[#1D1F23]">{title}</p>
                <p className="font-medium text-[#40444D]">{message}</p>
            </div>
