...
<input id="strtype" name="strtype" type="hidden" value="{{\Auth::user()->accountType}}">
    ...
    @foreach($cocogen_marine_quote_sum as $cocogen_sum)
    ...
    <button type="button" id="btn_issued" name="btn_issued" class="btn btn-default"><span class=" glyphicon glyphicon-export" aria-hidden="true"></span> Issued ({{$cocogen_sum->quote_issued}})</button>
    <button type="button" id="btn_dnm" name="btn_dnm" class="btn btn-default"><span class="glyphicon glyphicon-ban-circle" aria-hidden="true"></span> DNM ({{$cocogen_sum->quote_dmn}})</button> 
...
@if(\Auth::user()->accountType == 'Sales' || \Auth::user()->accountType == 'Agent')
    ...
    <div class="input-group">
      <span class="input-group-addon">Select Template</span>
          <select id="template" name="template" class="form-control">
          <option value="MTR-I">MTR – For Individual</option>
          <option value="MTR-C">MTR – For Corporate</option>
          <option value="MCI-I">MCI – For Individual</option>
          <option value="MCI-C">MCI – For Corporate</option>
    </select>
      <span class="input-group-btn">
      <button type="button" id="btn_addnew" name="btn_addnew" class="btn btn-default"><span class="glyphicon glyphicon-plus-sign" aria-hidden="true"></span> Add New</button>  
      </span>
  ...
  <script>
   ...
   $.ajaxSetup({
    ...
    $('#quote_table').DataTable({pageLength: 25,
           processing: true,
           serverSide: true,
           ajax: "{{ url('/get-quote/getallmarinequote') }}",
           type: "post",
           columns: [
                    { data: 'id', name: 'id'},
                    { data: 'assured', name: 'assured' },
                    { data: 'subline', name: 'subline' },
...
$('#btn_review').click(function(e){
  ...
  $(document).on('click', '.view_quote', function(){
        ...
        window.location.href = "{{ url('/get-quote/marine-quoteview')}}"+"/"+trans_id;
  ...
  $('#btn_addnew').click(function(e){
        ...
        //var SITEURL = '{{URL::to('')}}';
        //var trans_id =  $(this).attr('id');
        var trans_id = $("select[name='template']").val();

        window.location.href = "{{ url('/get-quote/marine-quote')}}"+"/"+trans_id;
