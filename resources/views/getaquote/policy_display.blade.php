...
<body>
    <div class="product content-wrapper bg-layout5">
        <section class="banner banner-product">
            ...
            <div class="container">
                ...
                <table class="table table-bordered data-table display" id='example' style="width: 100%;">
                    <thead>
                        <tr>
    ...
    <div class="modal-dialog modal-sm">
      <div class="modal-content">
        ...
        <form id="spoiledForm" action="{{ route('updateStatus') }}" method="POST">
            @csrf
            <p>Are you sure you want to change the status to Spoiled?</p>
...
<script type="text/javascript">
 jQuery(document).ready(function($) {
    ...
    columns: [
        ...
        { data: 'status', name: 'status' },
        {
            ...
            render: function(data, type, row) {
                ...
                if (row.status != 'Spoiled') {
                    // spoilbutton ="<button class='btn btn-danger spoiled-button'  data-row-id='" + row.id + "'>SPOILED</button>";
                    spoilbutton = '<button type="button" class="btn btn-danger btn-xs spoiled-button" data-row-id="' + row.id + '" data-toggle="modal" data-target="#myModal"><i class="fa fa-pencil fa-xs" aria-hidden="true"></i></button>';
    ...
    $('.data-table').on('click', 'button.spoiled-button', function(e) {
        e.preventDefault();
        var rowId = $(this).data('row-id');
        // Pass the row ID to the modal
        $('#spoiledModal').data('row-id', rowId);
        $('#rowIdInput').val(rowId);
        // Trigger the modal
        $('#spoiledModal').modal('show');
    });
