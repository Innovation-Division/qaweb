...
@endif
  ...
  <form method="post" action="{{ route('internationalinsurancesave') }}" enctype="multipart/form-data">
        ...
        <input type="hidden" id="hdn_proposal" name="hdn_proposal" value="" autocomplete="off">
...
<script>
    $(document).ready(function(){
      ...
      function startUploadPWD() {
        const $loadingBar = $('#uploadID-loading-bar');
        const $loadingProgress = $('#uploadID-loading-progress');
      ...
      $('#promo').on('input', function() {
        ...
        var fullbdate =  monthNamesFull[dathbdaysummary.getMonth()] + " " + dahbdaysummary + ", " + yeahbdaysummary; 
                    ...
                    var inprogress = '<div class="frame-108563">\
                      ...
                      <div class="frame-108567">\
                        <div class="trip-details payment-label">Payment</div>\
    ...
    $(document).ready(function(){
      ...
      $("#first_next").click(function(e){
        ...
        $.ajax({
          ...
          data:{ _token:$('input[name="_token"]').val(),
                  ...
                  noofday:days,
                  fromdate:$('#destination_from').val(),
                  todate:$('#destination_to').val(),
              ...
              success:function(result){
                    if ($.trim(result)){ 
                      jQuery.each(result, function(key, value){
                          ...
                          if($.trim(value['PlanName']) == 'Economy'){
                           
                            if(result['shengen'] == "Y"){
                       
                            }else{
                              $('#div-package-economy').show();
                              var prem_economy = parseFloat(value['Premium']);
                          ...
                          if(value['PlanName'] == 'Esteem'){
                            if(result['shengen'] == "Y"){
                            }else{
                              $('#div-package-esteem').show();
                              var prem_esteem = parseFloat(value['Premium']);
                          ...
                          if(value['PlanName'] == 'Executive'){
                            //alert(value['PlanName']);
                            var prem_executive = parseFloat(value['Premium']);
                          ...
                          if(value['PlanName'] == 'Elite'){
                            var prem_elite = parseFloat(value['Premium']);
      ...
      $("#personal_info_next").click(function(e){
        ...
        if($('input[name="hdn_covid_tag"]').val() == "Yes"){
          ...
          var benenames = [];
          var benemobile = [];
          var benerelationship = [];
          var emernames = [];
          var emermobile = [];
          var emerrelationship = [];
          // Capture all names
          $('input[name^="beneficiary-mobile"]').each(function() {
            benemobile.push($(this).val());
          });

          // Capture all ages
          $('input[name^="beneficiary-fullname"]').each(function() {
            benenames.push($(this).val());
          });

          // Capture all emails
          $('select[name^="beneficiary-relationship"]').each(function() {
            benerelationship.push($(this).val());
          });

          // Capture all names
          $('input[name^="emergency-mobile"]').each(function() {
            emermobile.push($(this).val());
          });

          // Capture all ages
          $('input[name^="emergency-fullname"]').each(function() {
            emernames.push($(this).val());
          });

          // Capture all emails
          $('select[name^="emergency-relationship"]').each(function() {
            emerrelationship.push($(this).val());
          });

        $.ajax({
          url:  $('input[name="url"]').val() + '/api/get-quote/itp-insurance/get-fullprem',
          method:"GET",
          data:{ _token:$('input[name="_token"]').val(),
                  cruise_tag:$('input[name="hdn_cruise_tag"]').val(),
                  covid_tag:$('input[name="hdn_covid_tag"]').val(),
                  ...
                  email:$('#email').val(),
                  gender:$('#gender').val(),
                  bday:$('#date-of-birth').val(),
                  bplace:$('#place-of-birth').val(),
                  id_type:$('#id_type').val(),
                  id_no:$('#id_no').val(),
                  houseno:$('#address-house-unit').val(),
                  street:$('#street').val(),
                  ...
                  pwdTag:$('input[name="hdn_pwd_tag"]').val(),
                  fromdate:$('#destination_from').val(),
                  todate:$('#destination_to').val(),
                  package: $('input[name="hdn_package"]').val(),
                  covidtype:$('input[name="hdn_covid_yes_type"]').val(),
                  destinations:destinations_list,
                  destinations_cruise:$('#destinations').val()
              },   error: function(data){
              ...
              success:function(result){
                console.log(result); 
                let ProposalNo = result.ProposalNo; //hdn_proposal
                let premium = result.BeforeVatPremium; //premium
                let DSTAmount = result.DSTAmount; //premium
                let PremiumTaxAmount = result.PremiumTaxAmount; //premium
                let LGTAmount = result.LGTAmount; //premium
                let DuePremium = result.DuePremium; // gross premium

                let dollarRate = $('input[name="hdn_dollar_rate"]').val(); 
                // Convert the dollar rate to a float
                let premiumPiso = dollarRate * DuePremium;
                $('input[name="hdn_proposal"]').val(ProposalNo); //hdn_dollar_rate
                
                $(".quote_netprem_dollar").text("$"+parseFloat(premium).toFixed(2));
                $(".quote_dst_dollar").text("$"+parseFloat(DSTAmount).toFixed(2));
                $(".quote_prem_tax_dollar").text("$"+parseFloat(PremiumTaxAmount).toFixed(2));
                $(".quote_lgt_dollar").text("$"+parseFloat(LGTAmount).toFixed(2));
                $(".quote_prem_dollar").text("$"+parseFloat(DuePremium).toFixed(2));
                $(".quote_prem_piso").text(ReplaceNumberWithCommas(premiumPiso).toFixed(2));
