...
use App\cocogen_getquote_mvtypes;
use App\cocogen_getquote_premium;
use App\cocogen_compre_personalinfo_changeprimary;
use App\province;
use App\cocogen_fmv;
use App\cocogen_brand_year;
use App\cocogen_compre_bipd;
use App\cocogen_compre_carinfo;
use App\cocogen_compre_personalinfo;
use App\cocogen_compre_addtlcarinfo;
use App\cocogen_compre_carinfo_changeprimary;
use App\cocogen_compre_accessory_trans;
use App\cocogen_compre_accessory;
use App\location_compre;
use App\cocogen_meta;
use App\cocogen_admin_subchild;
use App\cocogen_admin_main;
use App\cocogen_admin_submain;
use App\cocogen_admin_productlink;
use App\cocogen_admin_footer; 
use DB;
use Mail;
use Crazymeeks\Foundation\PaymentGateway\Dragonpay;
use App\cocogen_compre_trans_uploads;
use App\cocogen_itp_countries;
use App\location;
use Carbon\Carbon;
...
class compre extends Controller
{
                ...
                $cocogen_compre_personalinfo_changeprimary->save();  
                        ...
                        if($request->file('file')){
                            if ($request->file('file')->isValid()) { 
                                ...
                                $cocogen_compre_trans_uploads->filesize = $request->file('file')->getClientSize();
    ...
    public function carInformation(Request $request){
        ...
        if($request->page === 'firstPage') {
            ...
            if ($request->accessory) {
                ...
                $count = count($countsaccessory);
                for ($i = 0; $i < $count; $i++) {
                    if (!$request->accessories_value[$i]) {
                        $accamounts = 0;
                    } else {
                        $accamounts = str_replace(',', '', $request->accessories_value[$i]);
                    }
                    $amountAcct += $accamounts;
                }
            }
            ...
            if($request->buttonGuardValue === 'no') {
                ...
                $netPrem = $propertyDamagePrem->pd + $bodilyInjuryPrem->bi + $ODTheftPrem + 1500;
            }
            ...
            if($request->buttonGuardValue === 'no') {
                ...
                $netPremwithAOM = $propertyDamagePrem->pd + $bodilyInjuryPrem->bi + $ODTheftPrem + $actsOfNaturePrem + 1500;
            }
    ...
    public function personalData(Request $request)
    {
        ...
        if($request->page === 'additionalCar') {
            ...
            $cocogen_compre_addtlcarinfo->seatingNo = $request->capacity;
        ...
        if(!$request->uploadTrans === 'create') {
            if($request->file('upload')){
                if ($request->file('upload')->isValid()) { 
                    ...
                    $cocogen_compre_trans_uploads->filesize = $request->file('upload')->getClientSize();
