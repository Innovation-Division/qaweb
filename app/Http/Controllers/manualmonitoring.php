...
use DB;
use App\Models\Http\Controllers\PostsExport;
use Maatwebsite\Excel\Facades\Excel;
use Illuminate\Support\Facades\Hash;
use File;
use Mail;
use App\Models\cocogen_monitoring_production;
use App\Models\cocogen_monitoring_upload_file;
use App\Models\cocogen_users_agent_code;
use App\Models\cocogen_monitoring_record;
use App\Models\cocogen_monitoring_for_approval;
...
class manualmonitoring extends Controller
{
    public function import(Request $request)
    {
        ...
        if (Auth::check()) {
            ...
            if (count($request->file("file")) === 0) {
                ...
                $check_record = cocogen_monitoring_for_approval::select(
                    "status"
                )
                ...
                if ($request->file("file")) {
                    ...
                    $filesN = $request->file('file');
                    if ($filesN->isValid()) {
                        $totalCount = cocogen_monitoring_for_approval::select('intermediarycount')
                            ->where('intermediary', $request->intermediary)
                        ...
                        $dateToday = date('Y-m-d');
                        $cocogen_monitoring_production = new cocogen_monitoring_production;
                        $cocogen_monitoring_production->intermediaryno = $request->intermediary;
                        $cocogen_monitoring_production->addpad = $request->addpad;
                        $cocogen_monitoring_production->filename = $filename;
                        $cocogen_monitoring_production->filepath = $dateToday . '/' . $filename;
                        $cocogen_monitoring_production->filename = $filename;
                        $cocogen_monitoring_production->intermediarycount = @$totalCount[0]['intermediarycount'];
                        $cocogen_monitoring_production->save();
                        $file->move(storage_path('app/public') . '/monitoring/' . $dateToday . '/', $filename);
                        $prepareemail = cocogen_monitoring_production::where('intermediaryno', $request->intermediary)->get();
                        $location = storage_path('app/public/monitoring') . "/" . $prepareemail[0]["filepath"];
                        cocogen_monitoring_for_approval::where('intermediarycount', $totalCount[0]['intermediarycount'])
                            ->update(['dateupload' => $dateToday]);
                        $this->emailsattachment($request->padtype, $request->intermediary, $request->intermediaryname, $requestee, $location);// SALES SERVICE REQEUST
        ...
        $count_fileupload = cocogen_monitoring_upload_file::
            where("intermediaryno", $request->intermediary)
        ...
        $data = cocogen_monitoring_record::select("padtype", "padrange", "intermediary", "dateupload", "recordcount", "uploader")
            ...
            ->whereRaw('id IN (SELECT MAX(id) FROM cocogen_monitoring_record WHERE intermediaryno = ? AND intermediarycount = ? AND status IN (?, ?) GROUP BY intermediarycount)', [$request->intermediary, $intercount, 'APPROVE', 'RELEASE'])
            ->orderByDesc("id")
            ->get();
    ...
    public function checkApprove(Request $request)
    {
        $results = DB::table("cocogen_monitoring_for_approval")
            ->where("intermediary", $request->intermediary)
            ->whereNotNull('status')
            ->whereIn('status', ['APPROVED', 'RELEASED', 'ENDORSED', 'PENDING'])
            ->orderBy("intermediarycount", "desc")
            ->first(["used_doc", "status"]);

        if ($results) {
            $status = $results->status;
            $checkRelease = DB::table("cocogen_monitoring_for_approval")
                ->where("intermediary", $request->intermediary)
                ->where("status", "RELEASED")
                ->orderBy("intermediarycount", "desc")
                ->count();

            $showUploadButton = $checkRelease > 0 ? 1 : 0;

            return [$status, $showUploadButton];
        } else {
            return "NO REQUEST";
        }
