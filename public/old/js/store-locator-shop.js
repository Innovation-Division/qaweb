...
$(function() {      
    $('.region').change(function(){    // use class or use $('select')
        if ($(this).val() == "1") {
            $("#ab").show();
        } else {
             $("#ab").hide();
...
var dealersSearch = {
    initFunctions: function() {
        ...
        jQuery('.dealer-location-area').on('change', function() {
    ...
    itemContactOpen: function(searchItem) {
        if(checkIsMobile()) {
            if(!jQuery(searchItem).hasClass('active')) {
                jQuery('.dealer-open-seach').removeClass('dealer-open-search');
            }
        }
    ...
    searchClearOnChange: function() {
        jQuery('.dealer-location-input').val('');
    }
}
...
jQuery('select[name="region"]').on('change', function(e) {
    ...
    if (jQuery(this).val() == 0) {
         window.location.href = "https://www.cocogen.com/locate-a-branch"
    } else {
        window.location.href = "https://www.cocogen.com/locate-a-branch/" + jQuery(this).val();
    }

});

jQuery('select[name="regionshop"]').on('change', function(e) {
    var path = window.location.pathname;
    ...
    if (jQuery(this).val() == 0) {
         window.location.href = "https://www.cocogen.com/locate-a-shop"
    } else {
        window.location.href = "https://www.cocogen.com/locate-a-shop/" + jQuery(this).val();
    }
