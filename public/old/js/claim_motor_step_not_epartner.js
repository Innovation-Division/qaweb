	...
	btnnextclmmotor.click(function(){
	    	...
	    	errornumber = 0; 
	     	if(currentclmmotor == 1){
				...
				if($('#Loss_time').val() == ""){
					$("#Loss_time").after("<div class='validation_Loss_time v-error-msg'>Estimate Loss Time is empty</div>");    
			...
			if(currentclmmotor == 2){
				$(".validation_email_address_motor_not").remove();
	     		$(".validation_email_address_motor_not_check_error").remove(); 
	     		$(".validation_email_address_motor_not_check_success").remove();
				...
				$(".validation_tel_no_motor_not").remove();
	     		$(".validation_tel_no_motor_not_check_error").remove(); 
	     		$(".validation_tel_no_motor_not_check_success").remove();

				$(".validation_mobile_no_motor_not").remove();
	     		$(".validation_mobile_no_motor_not_check_error").remove(); 
	     		$(".validation_mobile_no_motor_not_check_success").remove();

				$(".validation_name_reportee_motor_not").remove();
	     		$(".validation_name_reportee_motor_not_check_error").remove(); 
	     		$(".validation_name_reportee_motor_not_check_success").remove();

				$(".validation_no_passenger_motor_not").remove();
	     		$(".validation_no_passenger_motor_not_check_error").remove(); 
	     		$(".validation_no_passenger_motor_not_check_success").remove();

				$(".validation_pwd_div_1").remove();
	     		$(".validation_pwd_div_1_check_error").remove(); 
	     		$(".validation_pwd_div_1_check_success").remove();

				$(".validation_mortgaged").remove();
	     		$(".validation_mortgaged_check_error").remove(); 
	     		$(".validation_mortgaged_check_success").remove();
				...
				if($('#email_address_motor_not').val() == ""){
					$("#email_address_motor_not").after("<div class='validation_email_address_motor_not v-error-msg'>Email address is empty</div>");    
					$('#email_address_motor_not').fieldErrorState();
					 errornumber = 1;           
				}else{	     			
					if(IsEmail($('#email_address_motor_not').val())==false){
						$("#email_address_motor_not").after("<div class='validation_email_address_motor_not v-error-msg'>Invalid format</div>");    
						$("#email_address_motor_not").fieldErrorState();   
						errornumber = 1;  
					}else{
						$('#email_address_motor_not').fieldSuccessState();
					}   	     			
					
				}  

				if($('#tel_no_motor_not').val() == ""){
					$("#tel_no_motor_not").after("<div class='validation_tel_no_motor_not v-error-msg'>Tel. no. is empty</div>");    
					$('#tel_no_motor_not').fieldErrorState();
					 errornumber = 1;           
				}else{	     				     			
					$('#tel_no_motor_not').fieldSuccessState();
				} 

				if($('#mobile_no_motor_not').val() == ""){
					$("#mobile_no_motor_not").after("<div class='validation_mobile_no_motor_not v-error-msg'>Mobile no. is empty</div>");    
					$('#mobile_no_motor_not').fieldErrorState();
					 errornumber = 1;           
				}else{	     				     			
					$('#mobile_no_motor_not').fieldSuccessState();
				} 

				if($('#name_reportee_motor_not').val() == ""){
					$("#name_reportee_motor_not").after("<div class='validation_name_reportee_motor_not v-error-msg'>Name of reportee is empty</div>");    
					$('#name_reportee_motor_not').fieldErrorState();
					 errornumber = 1;           
				}else{	     				     			
					$('#name_reportee_motor_not').fieldSuccessState();
				} 

				if($('#no_passenger_motor_not').val() == ""){
					$("#no_passenger_motor_not").after("<div class='validation_no_passenger_motor_not v-error-msg'>No./Who were the passenger is empty</div>");    
					$('#no_passenger_motor_not').fieldErrorState();
					 errornumber = 1;           
				}else{	     				     			
					$('#no_passenger_motor_not').fieldSuccessState();
				} 

				if($('#mortgaged').val() == ""){
					$("#mortgaged").after("<div class='validation_mortgaged v-error-msg'>The account has been mortgaged to is empty</div>");    
					$('#mortgaged').fieldErrorState();
					 errornumber = 1;           
				}else{	     				     			
					$('#mortgaged').fieldSuccessState();
				} 


				if($('input[name=hd_third_party_not]').val() == "no" || $('input[name=hd_third_party_not]').val() == "yes"){
					$("#lbl_third_party_motor_not").attr('style',  'color:#373737');
					$("#claim_motor_aget_yes_not").attr('style',  'border: 1px solid #C0C0C0;');
					$("#claim_motor_aget_no_not").attr('style',  'border: 1px solid #C0C0C0;');
					if($('input[name=hd_third_party_not]').val() == "yes"){
					...
					$("#lbl_third_party_motor_not").attr('style',  'color:#CF3C4B');
					$("#claim_motor_aget_yes_not").attr('style',  'border: 1px solid #CF3C4B;');
					$("#claim_motor_aget_no_not").attr('style',  'border: 1px solid #CF3C4B;');
					 errornumber = 1;           
				} 
			}
		...
		if(clmspacurrent < widgetclmspa.length) {
			...
			if(clmspacurrent == 1){
				...
				if($('#location_loss_pa').val() == ""){
					$("#location_loss_pa").after("<div class='validation_location_loss_pa v-error-msg'>Location of Loss is empty</div>");    
